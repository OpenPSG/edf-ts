...
export class EDFReader {
    ...
    readHeader() {
        ...
        const startTime = parseDate(`${startDateStr} ${startTimeStr}`, "dd.MM.yy HH.mm.ss", new Date());
        const headerBytes = parseInt(headerText.substring(184, 192).trim());
        const reserved = headerText.substring(192, 236).trim();
        const discontinuous = reserved.startsWith("EDF+D");
    ...
    readSignal(signalIndex, recordNumber) {
        ...
        const recordSize = header.signals.reduce((sum, s) => sum + s.samplesPerRecord * 2, 0);
        const startRecord = recordNumber ?? 0;
        const endRecord = recordNumber !== undefined ? recordNumber + 1 : header.dataRecords;
    ...
    readAnnotations(recordNumber) {
        const header = this.header ?? this.readHeader();
        const annSignalIndex = header.signals.findIndex((sig) => sig.label.includes("EDF Annotations"));
        if (annSignalIndex === -1)
            return [];
        const annotations = [];
        const offset = header.headerBytes;
    ...
    readFieldText(signalCount, start, end, signalIndex) {
        ...
        return this.textDecoder.decode(this.byteArray.subarray(offset, offset + (end - start)));
    }
